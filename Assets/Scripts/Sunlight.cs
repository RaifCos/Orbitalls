...
public class Sunlight : MonoBehaviour {
    ...
    private readonly HashSet<PlanetOld> litPlanets   = new();

    private void FixedUpdate() { foreach (var planet in candidates) ValidateSunlight(planet); }

    private void OnTriggerEnter2D(Collider2D other) { if (other.TryGetComponent<PlanetOld>(out var planet)) candidates.Add(planet); }
    ...
    private void ValidateSunlight(PlanetOld planet) {
    ...
    private void SetLit(PlanetOld planet, bool lit) {
